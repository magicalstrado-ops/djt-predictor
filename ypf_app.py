...
# ── FINNHUB ───────────────────────────────────────────────────────────────────
FINNHUB_KEY = "d6h85h1r01qnjncnn1r0"
FINNHUB_BASE = "https://finnhub.io/api/v1"

@st.cache_data(ttl=600, show_spinner=False)
def get_finnhub_news(ticker: str, days: int = 7):
    """Obtiene noticias recientes del ticker desde Finnhub."""
    try:
        hoy  = datetime.date.today()
        desde = (hoy - datetime.timedelta(days=days)).strftime("%Y-%m-%d")
        hasta = hoy.strftime("%Y-%m-%d")
        url = f"{FINNHUB_BASE}/company-news?symbol={ticker}&from={desde}&to={hasta}&token={FINNHUB_KEY}"
...
@st.cache_data(ttl=600, show_spinner=False)
def get_finnhub_sentiment(ticker: str):
    """Obtiene score de sentimiento de noticias desde Finnhub."""
    try:
        url = f"{FINNHUB_BASE}/news-sentiment?symbol={ticker}&token={FINNHUB_KEY}"
        r = requests.get(url, timeout=8)
        if r.status_code == 200:
            return r.json()
    except:
...
def get_finnhub_perfil(ticker: str):
    """Obtiene perfil de empresa desde Finnhub."""
    try:
...
# ── PANEL FINNHUB · NOTICIAS + SENTIMIENTO ────────────────────────────────────
st.markdown("<div class='etiqueta-seccion'>📰 NOTICIAS &amp; SENTIMIENTO · FINNHUB REAL-TIME</div>", unsafe_allow_html=True)

st.markdown("""<style>
.fh-sent-box {
    background: var(--navy3); border: 1px solid var(--border2);
    border-radius: 3px; padding: 18px 24px; margin-bottom: 18px;
    display: flex; align-items: center; gap: 28px; flex-wrap: wrap;
}
.fh-sent-kpi { text-align: center; min-width: 90px; }
.fh-sent-val { font-family:'JetBrains Mono',monospace; font-size: 1.4rem; font-weight: 700; line-height: 1; }
.fh-sent-lbl { font-family:'JetBrains Mono',monospace; font-size: 0.52rem; color: var(--texto-xs);
               letter-spacing: 0.16em; text-transform: uppercase; margin-top: 4px; }
.fh-gauge-wrap { flex: 1; min-width: 200px; }
.fh-gauge-track { background: #0d1626; border-radius: 2px; height: 6px; width: 100%; overflow: hidden; margin-top: 6px; }
.fh-gauge-fill  { height: 100%; border-radius: 2px; transition: width 0.6s ease; }
.fh-noticia {
    background: var(--navy2); border: 1px solid var(--border);
    border-radius: 3px; padding: 12px 16px; margin-bottom: 6px;
    transition: background 0.15s, border-color 0.15s;
    animation: fadeIn 0.3s ease both;
}
.fh-noticia:hover { background: var(--navy3); border-color: var(--border2); }
.fh-titular {
    font-family:'JetBrains Mono',monospace; font-size: 0.72rem;
    color: var(--texto); font-weight: 500; line-height: 1.45;
    text-decoration: none; display: block; margin-bottom: 5px;
}
.fh-titular:hover { color: var(--acento2); }
.fh-meta { font-family:'JetBrains Mono',monospace; font-size: 0.54rem; color: var(--texto-xs); letter-spacing: 0.06em; }
.fh-source { color: var(--acento); margin-right: 10px; }
.fh-badge-bull { display:inline-block; font-size:0.48rem; letter-spacing:0.12em; text-transform:uppercase;
    background:rgba(5,216,144,0.1); border:1px solid rgba(5,216,144,0.3); color:#05d890;
    padding:2px 7px; border-radius:2px; margin-left:8px; vertical-align:middle; }
.fh-badge-bear { display:inline-block; font-size:0.48rem; letter-spacing:0.12em; text-transform:uppercase;
    background:rgba(244,63,94,0.1); border:1px solid rgba(244,63,94,0.3); color:#f43f5e;
    padding:2px 7px; border-radius:2px; margin-left:8px; vertical-align:middle; }
.fh-badge-neu  { display:inline-block; font-size:0.48rem; letter-spacing:0.12em; text-transform:uppercase;
    background:rgba(59,130,246,0.08); border:1px solid rgba(59,130,246,0.2); color:#60a5fa;
    padding:2px 7px; border-radius:2px; margin-left:8px; vertical-align:middle; }
.fh-empty { font-family:'JetBrains Mono',monospace; font-size:0.62rem; color:#1e3050;
    padding:24px; text-align:center; border:1px solid #0d1626; border-radius:3px; }
</style>""", unsafe_allow_html=True)

_ticker_fh = ticker if not modo_arg else ticker  # Finnhub usa símbolos NYSE/NASDAQ

with st.spinner("📡  Consultando Finnhub..."):
    _sentiment = get_finnhub_sentiment(_ticker_fh)
    _noticias  = get_finnhub_news(_ticker_fh, days=7)
    _perfil    = get_finnhub_perfil(_ticker_fh)

# — Bloque de sentimiento —
_col_s1, _col_s2 = st.columns([1, 2])

with _col_s1:
    if _sentiment and isinstance(_sentiment, dict) and _sentiment.get("buzz"):
        _buzz        = _sentiment.get("buzz", {})
        _sent_data   = _sentiment.get("sentiment", {})
        _bull_pct    = _sent_data.get("bullishPercent", 0) * 100
        _bear_pct    = _sent_data.get("bearishPercent", 0) * 100
        _score       = _sentiment.get("companyNewsScore", 0)
        _articles    = _buzz.get("articlesInLastWeek", 0)
        _buzz_val    = _buzz.get("buzz", 0)

        _score_color = "#05d890" if _bull_pct > 60 else ("#f43f5e" if _bear_pct > 60 else "#60a5fa")
        _score_label = "ALCISTA" if _bull_pct > 60 else ("BAJISTA" if _bear_pct > 60 else "NEUTRO")

        st.markdown(f"""
        <div class='fh-sent-box' style='flex-direction:column;gap:14px;'>
            <div style='font-family:JetBrains Mono,monospace;font-size:0.52rem;color:#3d5a80;letter-spacing:0.2em;text-transform:uppercase;'>
                Sentimiento · últimos 7 días
            </div>
            <div style='display:flex;gap:24px;width:100%;justify-content:space-around;'>
                <div class='fh-sent-kpi'>
                    <div class='fh-sent-val' style='color:{_score_color};'>{_score_label}</div>
                    <div class='fh-sent-lbl'>Señal</div>
                </div>
                <div class='fh-sent-kpi'>
                    <div class='fh-sent-val' style='color:#05d890;'>{_bull_pct:.0f}%</div>
                    <div class='fh-sent-lbl'>Alcista</div>
                </div>
                <div class='fh-sent-kpi'>
                    <div class='fh-sent-val' style='color:#f43f5e;'>{_bear_pct:.0f}%</div>
                    <div class='fh-sent-lbl'>Bajista</div>
                </div>
                <div class='fh-sent-kpi'>
                    <div class='fh-sent-val' style='color:#60a5fa;'>{_articles}</div>
                    <div class='fh-sent-lbl'>Artículos</div>
                </div>
            </div>
            <div class='fh-gauge-wrap' style='width:100%;'>
                <div style='display:flex;justify-content:space-between;font-family:JetBrains Mono,monospace;font-size:0.48rem;color:#3d5a80;margin-bottom:3px;'>
                    <span>BEAR</span><span>BULL</span>
                </div>
                <div class='fh-gauge-track'>
                    <div class='fh-gauge-fill' style='width:{_bull_pct:.0f}%;background:linear-gradient(90deg,#05d890,#34d399);'></div>
                </div>
            </div>
        </div>""", unsafe_allow_html=True)
    else:
        if modo_arg:
            st.markdown("<div class='fh-empty'>Sentimiento no disponible<br><span style='font-size:0.52rem;color:#1e3050;'>Finnhub cubre principalmente tickers NYSE/NASDAQ</span></div>", unsafe_allow_html=True)
        else:
            st.markdown("<div class='fh-empty'>Sin datos de sentimiento para este ticker</div>", unsafe_allow_html=True)

with _col_s2:
    if _perfil and _perfil.get("name"):
        _p = _perfil
        st.markdown(f"""
        <div class='fh-sent-box' style='gap:16px;'>
            <div>
                <div style='font-family:JetBrains Mono,monospace;font-size:0.78rem;color:#f0f6ff;font-weight:600;'>{_p.get('name','')}</div>
                <div style='font-family:JetBrains Mono,monospace;font-size:0.54rem;color:#3d5a80;margin-top:3px;letter-spacing:0.08em;'>
                    {_p.get('exchange','')} · {_p.get('finnhubIndustry','')} · {_p.get('country','')}
                </div>
            </div>
            <div class='fh-sent-kpi'>
                <div class='fh-sent-val' style='color:#60a5fa;font-size:0.9rem;'>{_p.get('currency','')}</div>
                <div class='fh-sent-lbl'>Moneda</div>
            </div>
            <div class='fh-sent-kpi'>
                <div class='fh-sent-val' style='color:#f59e0b;font-size:0.9rem;'>${_p.get("marketCapitalization",0)/1000:.1f}B</div>
                <div class='fh-sent-lbl'>Mkt Cap</div>
            </div>
            <div class='fh-sent-kpi'>
                <div class='fh-sent-val' style='color:#a78bfa;font-size:0.9rem;'>{_p.get("shareOutstanding",0)/1000:.1f}B</div>
                <div class='fh-sent-lbl'>Acciones</div>
            </div>
        </div>""", unsafe_allow_html=True)

# — Noticias —
st.markdown("<div style='font-family:JetBrains Mono,monospace;font-size:0.52rem;color:#3d5a80;letter-spacing:0.2em;text-transform:uppercase;margin:10px 0 10px 0;'>ÚLTIMAS NOTICIAS · {}</div>".format(ticker), unsafe_allow_html=True)

if _noticias:
    for _n in _noticias:
        _ts      = datetime.datetime.fromtimestamp(_n.get("datetime", 0)).strftime("%d %b · %H:%M") if _n.get("datetime") else ""
        _src     = _n.get("source", "")
        _titulo  = _n.get("headline", "Sin título")[:130]
        _url_n   = _n.get("url", "#")
        _summ    = _n.get("summary", "")[:160]

        # Heurística de sentimiento simple por título
        _pos_words = ["surge","jumps","rises","rally","beat","profit","growth","record","upgrade","buy"]
        _neg_words = ["falls","drops","decline","loss","miss","downgrade","sell","crash","cut","warn"]
        _tit_lower = _titulo.lower()
        if any(w in _tit_lower for w in _pos_words):
            _sent_badge = "<span class='fh-badge-bull'>BULL</span>"
        elif any(w in _tit_lower for w in _neg_words):
            _sent_badge = "<span class='fh-badge-bear'>BEAR</span>"
        else:
            _sent_badge = "<span class='fh-badge-neu'>NEUTRO</span>"

        st.markdown(f"""
        <div class='fh-noticia'>
            <a class='fh-titular' href='{_url_n}' target='_blank'>{_titulo}{_sent_badge}</a>
            <div class='fh-meta'>
                <span class='fh-source'>{_src}</span>
                <span>{_ts}</span>
            </div>
            {"<div style='font-family:JetBrains Mono,monospace;font-size:0.6rem;color:#3d5a80;margin-top:5px;line-height:1.5;'>" + _summ + "…</div>" if _summ else ""}
        </div>""", unsafe_allow_html=True)
else:
    if modo_arg:
        st.markdown("<div class='fh-empty'>Sin noticias disponibles en Finnhub para este ticker<br><span style='font-size:0.52rem;color:#1e3050;'>Las acciones argentinas (ADRs) pueden tener cobertura limitada</span></div>", unsafe_allow_html=True)
    else:
        st.markdown("<div class='fh-empty'>Sin noticias recientes disponibles para este ticker</div>", unsafe_allow_html=True)
...
_pred_key = f"{ticker}_{datetime.datetime.now().strftime('%Y-%m-%d_%H')}_{datetime.datetime.now().minute // 10}"
